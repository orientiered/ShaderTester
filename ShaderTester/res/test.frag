...
vec2 coordTransform(vec2 a) {
	return (a * 2. - iResolution.xy) / iResolution.y;
}
...
void main()
{
    ...
    for( int n=0; n<AA; n++ )
    {
        ...
        vec2 p = coordTransform(gl_FragCoord.xy+offset+o) / globalScale;
        #else    
        vec2 p = coordTransform(gl_FragCoord.xy+offset) / globalScale;
        #endif
